...
class Environment {  // Environmental fields that determine storm strength and steering
    ...
    init(data) {
        if (data instanceof LoadData && data.format < saveFormat.WITH_IMPROVED_ENV) {   // Hardcoded conversion of data structure to Format 3 (doesn't affect values, thus old format number should cascade)
            let newData = {};
            let v = data.value;
            let o = (...d) => {
                return {
